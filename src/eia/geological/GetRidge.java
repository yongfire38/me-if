...
public class GetRidge {
	...
	final static Logger logger = Logger.getLogger(GetRidge.class);
	...
	public static void main(String[] args) {
		...
		if (args.length == 1) {
			...
			File file = new File(JsonParser.getProperty("file_path") + "GeologicalService_getRidge_" + strDate + ".dat");
			...
			try {
				...
				if (resultCode.equals("00")) {
					...
					for (int i = 0; i < ridges.size(); i++) {
						...
						while (iter.hasNext()) {
							...
							if (keyname.equals("ridgeNm")) {
								ridgeNm = ridge.get(keyname).toString();
							}
							if (keyname.equals("ridgeDstnc")) {
								ridgeDstnc = ridge.get(keyname).toString();
							}
					...
					File f = new File(JsonParser.getProperty("file_path") + "GeologicalService_getRidge_" + strDate + ".dat");
