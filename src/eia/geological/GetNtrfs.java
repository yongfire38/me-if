...
public class GetNtrfs {
	...
	final static Logger logger = Logger.getLogger(GetNtrfs.class);
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 1) {
			...
			File file = new File(JsonParser.getProperty("file_path") + "GeologicalService_getNtrfs_" + strDate + ".dat");
			...
			try {
				...
				if (resultCode.equals("00")) {
					...
					for (int i = 0; i < ntrfss.size(); i++) {
						...
						while (iter.hasNext()) {
							...
							if (keyname.equals("ntrfsNm")) {
								ntrfsNm = ntrfss_json.get(keyname).toString();
							}
							if (keyname.equals("ntrfsNo")) {
								ntrfsNo = ntrfss_json.get(keyname).toString();
							}
					...
					File f = new File(JsonParser.getProperty("file_path") + "GeologicalService_getNtrfs_" + strDate + ".dat");
