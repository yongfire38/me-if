	...
	public static void main(String[] args) throws Exception {
			...
			try {
				...
				if (args.length == 0) {
					...
					// 2020.06.05 : String 리턴으로 잡았더니 에러 남.. JSONObject리턴으로 수정하고, 해당 메서드에 빈 json 로직을 넣음 (하나 새로 뺌)
					/*if(json.indexOf("</") > -1){
						System.out.print("공공데이터 서버 비 JSON 응답");
						json ="{\"getRadioActiveMaterList\":{\"header\":{\"code\":\"03\",\"message\":\"NODATA_ERROR\"}}}";
					}*/

					JSONObject count_obj = JsonParser.parsePriJson_radioActive_obj(service_url, service_key, String.valueOf(pageNo));
					...
					for (int i = 1; i <= pageCount; i++) {
						...
						JSONObject obj = JsonParser.parsePriJson_radioActive_obj(service_url, service_key, String.valueOf(i));
