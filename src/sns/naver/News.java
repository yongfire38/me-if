						...
						if(error != null){
							...
							for (int i = 1; i <= pageCount; i++) {
								...
								if (i % 100 == 1 && i <= 101) {
									...
									String title = " "; // 뉴스 제목
									String originallink = " "; // 뉴스원본
																						// 링크
									String link = " "; // 네이버 뉴스 링크
									String description = " "; // 뉴스 내용
									String pubDate = " "; // 제공일시
									...
									for (int r = 0; r < items.size(); r++) {
										...
										while (iter.hasNext()) {
											...
											if(keyname.equals("originallink")) {
												if(!(JsonParser.isEmpty(item.get(keyname)))){
													originallink = item.get(keyname).toString().replaceAll("(\r\n|\r|\n|\n\r)", " ")
															.replace("'", "").replace("&#39;", "").replace("&#34;", "")
															.replaceAll("(\\s{2,}|\\t{2,})", " ");
													
													// 에러 유발자들인 emoji 제거..
													Pattern emoticons = Pattern.compile("[\\uD83C-\\uDBFF\\uDC00-\\uDFFF]+");
													Matcher emoticonsMatcher = emoticons.matcher(originallink);
													originallink = emoticonsMatcher.replaceAll(" ").replaceAll("\\p{InEmoticons}+", "")
															.replaceAll("\\p{So}+", "").replaceAll("\\p{InMiscellaneousSymbolsAndPictographs}+", "");
											...
											if(keyname.equals("pubDate")) {
												if(!(JsonParser.isEmpty(item.get(keyname)))){
													pubDate = item.get(keyname).toString().replaceAll("(\r\n|\r|\n|\n\r)", " ")
															.replace("'", "").replace("&#39;", "").replace("&#34;", "")
															.replaceAll("(\\s{2,}|\\t{2,})", " ");
													
													// 에러 유발자들인 emoji 제거..
													Pattern emoticons = Pattern.compile("[\\uD83C-\\uDBFF\\uDC00-\\uDFFF]+");
													Matcher emoticonsMatcher = emoticons.matcher(pubDate);
													pubDate = emoticonsMatcher.replaceAll(" ").replaceAll("\\p{InEmoticons}+", "")
															.replaceAll("\\p{So}+", "").replaceAll("\\p{InMiscellaneousSymbolsAndPictographs}+", "");
