...
public class News {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 1 || args.length == 2) {
			...
			json = JsonParser.parseBlogJson_naver(service_url, naver_client_id, naver_client_secret, args[0], job_dt, "1");
			...
			StringBuffer originallink = new StringBuffer(" "); // 뉴스원본 링크
			...
			for (int i = 1; i <= pageCount; i++) {
				...
				// 여기서의 i는 페이지 넘버가 아닌 조회 시작 위치값이므로 페이지 당 표시 가능 수인 100만큼 증가, 1000
				// 초과값은 api가 지원하지 않음
				if (i % 100 == 1 && i <= 1000) {

					System.out.println("페이지 검색 시작 위치는:::" + i);

					json = JsonParser.parseBlogJson_naver(service_url, naver_client_id, naver_client_secret, args[0], job_dt,
							Integer.toString(i));
