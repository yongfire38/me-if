			...
			try {
				...
				if (args.length == 3) {
					...
					if (args[1].length() == 6 && args[2].length() == 6) {
						...
						if(json.indexOf("</") > -1){
							System.out.print("공공데이터 서버 비 JSON 응답 , code :" + args[0]+ ", stdt :" + args[1]+ ", eddt :" + args[2]);
				...
				e.printStackTrace();
				System.out.println("클래스명 : "+ Thread.currentThread().getStackTrace()[1].getClassName() + ", code :" + args[0]+ ", stdt :" + args[1]+ ", eddt :" + args[2]);
				System.exit(-1);
			}
