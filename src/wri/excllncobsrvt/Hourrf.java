...
public class Hourrf {
	...
	public static void main(String[] args) throws Exception {
		...
		// 필요한 파라미터는 조회시작일 (yyyyMMdd), 조회시작시각 2자리, 조회 종료일 (yyyyMMdd), 조회종료 시각 2자리, 댐 코드, 우량관측소 코드
		...
		if (args.length == 6) {
			...
			if (args[0].length() == 8 && args[1].length() == 2 && args[2].length() == 8 && args[3].length() == 2) {
				...
				json = JsonParser.parseWriJson_excll(service_url, service_key, String.valueOf(pageNo), args[0], args[1], args[2], args[3], args[4], args[5]);
				...
				for (int i = 1; i <= pageCount; i++) {
					...
					json = JsonParser.parseWriJson_excll(service_url, service_key, String.valueOf(i), args[0], args[1], args[2], args[3], args[4], args[5]);
