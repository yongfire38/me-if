...
public class Hourwal {

	final static Logger logger = Logger.getLogger(Hourwal.class);

	// 우량 관측정보 조회 서비스 - 수위 시강우량 조회
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 6) {
			...
			if (args[0].length() == 8 && args[1].length() == 2 && args[2].length() == 8 && args[3].length() == 2) {
				...
				String service_url = JsonParser.getProperty("excllncobsrvt_hourwal_url");
				String service_key = JsonParser.getProperty("excllncobsrvt_service_key");

				// step 1.파일의 첫 행 작성
				File file = new File(JsonParser.getProperty("file_path") + "WRI/TIF_WRI_15_" + args[0] + "_" + args[1]
				...
				TransSftp.transSftp(JsonParser.getProperty("file_path") + "WRI/TIF_WRI_15_" + args[0] + "_" + args[1]
