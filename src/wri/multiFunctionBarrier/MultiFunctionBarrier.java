...
public class MultiFunctionBarrier {
	...
	final static Logger logger = Logger.getLogger(MultiFunctionBarrier.class);
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 4) {
			...
			if (args[0].length() == 8 && args[1].length() == 8 && args[2].length() == 8 && args[3].length() == 2) {
				...
				// step 1.파일의 첫 행 작성
				File file = new File(JsonParser.getProperty("file_path") + "WRI/TIF_WRI_11_" + args[0] + "_" + args[1]
				...
				StringBuffer nowprcptqy = new StringBuffer(" "); // 누계 금년
				StringBuffer lastprcptqy = new StringBuffer(" "); // 누계 전년
				StringBuffer nyearprcptqy = new StringBuffer(" "); // 누계 예년
				StringBuffer inflowqy = new StringBuffer(" "); // 전입유입량
				StringBuffer totdcwtrqy = new StringBuffer(" "); // 방류량 계
				StringBuffer fishway = new StringBuffer(" "); // 방류량 어도
				StringBuffer dvlpdcwtrqy = new StringBuffer(" "); // 방류량 소수력
				...
				TransSftp.transSftp(JsonParser.getProperty("file_path") + "WRI/TIF_WRI_11_" + args[0] + "_" + args[1]
